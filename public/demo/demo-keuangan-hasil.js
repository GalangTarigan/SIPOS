...
$(function(){
    ...
    $.ajax({
        url: "/admin/keuangan/daftar-laporan-penjualan/?tanggal=" + mulai + "&sampai=" + selesai, 
        type: 'get',
        data: {
            "_token" : token,              
        },
...
//daterangepicker
    ...
    $('#cetakLaporan').on('click', function (e) {
        ...
        if ( ! table1.data().any() ) {
            Swal.fire({
                title: 'Oops...',
                text: 'Tidak ada data pada tabel, silahkan pilih tanggal lainnya.',
                width: '400px',
                type: 'info',
                backdrop: `rgba(0,0,123,0.4)`
              })
        }else{            
            window.open('/print/laporan-penjualan/?tanggal=' + mulai + "&sampai=" + selesai, 'print', 'scrollbars=yes,status=no,menubar=no,width=800,height=400')
        }
